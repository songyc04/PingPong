    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      if (packetSize) {
        ...
        if (response == "END" || response == "STOP") {
          ...
          gState         = STATE_SRT;
          joystickActive = false;  // 3 2 1 카운트다운 동안 전송 차단 유지
          requestLCD("");          // 화면 클리어
        }
        else if (response == "score the goal!") {
          // ⭐ [해결 3] 설정창에서 바로 복귀할 때 멈추는 문제 방지용 강제 상태 전이
          gState         = STATE_SRT; 
          joystickActive = true;
          tLastSend      = millis();
        }
        ...
        // ⭐ [해결 4] 'score the goal!'을 필터링 명단에서 제외하여 LCD에 정상 출력되도록 수정
...
void loop() {
  ...
  if (buttonClicked) {
    ...
    if (gState == STATE_END || gState == STATE_SRT) {
      sendUDP("SET\n");
      // ⭐ [해결 2] 게임 중 설정창 진입 시 확실한 메뉴 조작을 위해 로컬 상태 강제 전이 추가
      gState         = STATE_SET;
      joystickActive = false;
      setMenuCursor  = 0;
      resetMenuFlags = true;
  ...
  if (gState == STATE_END) {
    ...
    // ⭐ [해결 1] 잠금 플래그(srtRequestSent) 삭제. 파이썬 서버 응답 기반으로만 전이 허용.
    if (isUp && !lastUpState) {
      sendUDP("SRT\n");
    }
    lastUpState = isUp;
