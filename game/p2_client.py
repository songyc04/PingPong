...
def recv_thread():
	...
	try:
		...
		print(f"[P2 수신] 포트 {RECV_PORT} 개방 완료")
	except Exception as e:
		print(f"[P2 수신] 포트 바인딩 실패: {e}")
		return
		...
		try:
			...
			if data:
				...
				if msg == "SRT":
					...
					print("[P2] 조이스틱 시뮬레이션 시작")
				elif msg in ["END", "STP"]:
					with joystick_lock:
						joystick_active = False
					print("[P2] 조이스틱 시뮬레이션 중지")
...
def send_message(msg):
	try:
		...
		print(f"[P2 송신 -> Python:{SEND_PORT}] {msg}")
	except Exception as e:
		print(f"[P2 송신] 오류: {e}")
...
def joystick_thread():
	...
	while True:
		...
		if active:
			...
			try:
				tx_socket.sendto(msg.encode("utf-8"), (SERVER_IP, SEND_PORT))
				print(f"[P2 조이스틱 자동 전송] {msg}")
			except Exception as e:
				print(f"[P2 조이스틱] 전송 오류: {e}")


def main():
	print("=" * 50)
	print("  P2 ESP32 시뮬레이터 클라이언트")
	print("=" * 50)
