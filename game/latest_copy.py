...
#    global p1_joy_x, p1_joy_y, p2_joy_x, p2_joy_y
#    global game_time_limit, time_limit_idx
...
#       if current_cmd:
#          # SRT 수신 시에는 3초 카운트다운 완료 후 송신하므로 즉시 던지지 않습니다.
#          if current_cmd != "SRT":
...
def udp_server_thread():
   ...
   UDP 수신 전용 스레드 (10001번 포트 강제 바인딩)
   """
   global network_command, last_joystick_ip, countdown_active
   SERVER_IP = "0.0.0.0" # 모든 네트워크 인터페이스에서 수신
   ...
   while True:
      try:
         ...
         if ',' in raw_msg:
            # [핵심 수정] 카운트다운이 진행 중일 때는 하드웨어 조이스틱 아날로그 패킷 파싱을 무시합니다.
            if UI_state == "GAME_PLAY" and countdown_active:
               continue
            with command_lock:
               parse_joystick(raw_msg)
   ...
   while running:
      ...
      if UI_state == "GAME_PLAY" and countdown_active:
         ...
         if countdown_timer <= 0:
            ...
            # 조이스틱 초깃값 리셋 후 전송
            p1_joy_x, p1_joy_y, p2_joy_x, p2_joy_y = 0.0, 0.0, 0.0, 0.0
            send_to_esp32("SRT")  # 카운트다운 종료 시점에 정확히 SRT 전송
            game_timer_active = True
      ...
      # --- 외부 수신 명령 처리 및 ESP32 바이패스 송신 ---
      if current_cmd:
         # [핵심 수정] 하드웨어 패널에서 SRT를 직접 전송한 경우에도 3초 카운트다운 트리거를 걸어줍니다.
         if current_cmd == "SRT":
            if UI_state == "MAIN_MENU":
               p1_score, p2_score = 0, 0
               ball_x = WIDTH // 2 - ball_size // 2
            ...
            # SRT가 아닌 일반 명령(STP, END 등)만 즉시 바이패스 송신합니다.
            send_to_esp32(current_cmd)
