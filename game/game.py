...
def udp_server_thread():
	...
	SERVER_IP = "0.0.0.0" # 공유기 내부의 모든 수신 패킷 수용 허용
	rx_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
	
	try:
		rx_socket.bind((SERVER_IP, PYTHON_RCV_PORT))
		# ★ 수정: 소켓이 무한정 기다리지 않도록 0.001초 타임아웃(Non-blocking 효과) 부여
	...
	while True:
		try:
			...
			# ★ 수정: 데이터 판별 조건 고도화 (순수 조이스틱 패킷인지 메뉴 제어 문자열인지 완벽히 차단 필터링)
			if ',' in raw_msg and (raw_msg[0].isdigit() or raw_msg[0] == '-'):
				with command_lock:
					parse_joystick(raw_msg)
			else:
				# 콤마가 없는 순수 문자열 명령어(UP, DN, SRT, STP, SET 등) 처리
				print(f"[네트워크 수신 Command 수신 성공!]: {raw_msg}")
				with command_lock:
					network_command = raw_msg
					
		except socket.timeout:
			# 타임아웃 발생 시 무시하고 다음 루프로 넘어가 프로그램 멈춤을 방지
			continue
		except Exception as e:
